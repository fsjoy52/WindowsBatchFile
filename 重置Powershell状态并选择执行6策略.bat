...
echo 2. AllSigned (只允许运行签名脚本)
echo 3. RemoteSigned (允许运行本地脚本，远程脚本需要签名)
echo 4. Unrestricted (允许运行所有脚本，无限制)
echo 5. Bypass (绕过所有执行策略限制)
echo 6. Undefined (清除当前用户执行策略，继承系统范围策略)
set /p choice="请输入您的选择 (1/2/3/4/5/6): "
...
if "%choice%"=="1" (
    ...
    echo 已将执行策略设置为 Bypass。
) else if "%choice%"=="6" (
    powershell -Command "Set-ExecutionPolicy Undefined -Scope CurrentUser -Force"
    echo 已将执行策略设置为 Undefined。
) else (
    echo 无效的选择，请重新运行脚本并选择 1, 2, 3, 4, 5 或 6。
    pause
    exit /b
)
